...
# 1) Gather all Bluetooth sinks
mapfile -t sinks_arr < <(pactl list short sinks | awk '/bluez_sink/ {print $2}')
...
old_mod=$(pactl list short modules | awk '/module-combine-sink/ && /sink_name=bt_combined/ {print $1}')
if [ -n "$old_mod" ]; then
